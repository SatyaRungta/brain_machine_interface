...
for neuron_id in range(0,15,1):
    ...
    for pos in range(0,all_pos,1):
        ...
        plt.fill_between([-25, 75], [250, 250], 0, facecolor=[.8, .8, .8], alpha=0.2)
        plt.vlines(0,0,225,colors='k',linestyles='--')
        plt.xlim(-1250,250)
        plt.ylim(0,225)
        plt.xticks(ticks=list(range(-1250,250,250)),labels=list(range(-1250,250,250)))
        plt.xlabel('Time to saccade (in ms)')
    ...
    figname = 'Neuron' + str(neuron_id + 1) + '_Movement.jpg'
